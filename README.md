...
This demo demonstrates the basic development workflow for camera image acquisition on UniRTOS, and is suitable as a beginner example for camera peripheral debugging and image processing application development.

- Demonstrates camera module initialization and continuous image preview frame capture
- Supports optional QR code / barcode decoding via the built-in code decoder library
- Supports UART-triggered debug mode for on-demand frame output and analysis
- Easily extensible to image upload, edge detection, face detection, and other computer vision workflows
...
```bash
unirtos-cli new -r unirtos-camera-demos
```

To specify a version:

```bash
unirtos-cli new -r unirtos-camera-demos -v 1.0.0
```

### 3. Enter the Project and Build

```bash
cd unirtos-camera-demos-1.0.0
